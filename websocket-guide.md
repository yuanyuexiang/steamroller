...
确保您的环境变量正确配置：
```bash
DIRECTUS_URL=https://steamroller.kcbaotech.com
...
```yaml
traefik.http.routers.directus.rule: Host(`steamroller.kcbaotech.com`) && (PathPrefix(`/websocket`) || PathPrefix(`/graphql`) || PathPrefix(`/`))
```
...
```javascript
// WebSocket: 开始连接流程
// WebSocket: 尝试连接到Directus WebSocket端点: wss://steamroller.kcbaotech.com/websocket
// WebSocket: 连接已建立
...
1. **检查端点可用性**:
   ```bash
   curl -I https://steamroller.kcbaotech.com/websocket
   curl -I https://steamroller.kcbaotech.com/graphql
   ```
...
```json
{
  "primary": "wss://steamroller.kcbaotech.com/websocket",
  "fallback": "wss://steamroller.kcbaotech.com/graphql",
  "possibleEndpoints": [
    "wss://steamroller.kcbaotech.com/websocket",
    "wss://steamroller.kcbaotech.com/graphql"
  ],
  "authMode": "handshake",
  "directusUrl": "https://steamroller.kcbaotech.com",
  "success": true
}
```
