...
// 根据城市代码获取完整地址（省+市）
export const getFullAddressByCityCode = (cityCode: string): string => {
...
// 根据城市名称获取完整地址（省+市）
export const getFullAddressByCityName = (cityName: string): string => {
  for (const province of CHINA_PROVINCES) {
  ...
  return cityName; // 如果找不到，返回原始城市名
};

// 解析完整地址，返回省市信息
export const parseFullAddress = (fullAddress: string): { province?: Province; city?: City } => {
  if (!fullAddress) return {};
  
  for (const province of CHINA_PROVINCES) {
    if (fullAddress.includes(province.name)) {
      const city = province.cities.find(c => fullAddress.includes(c.name));
